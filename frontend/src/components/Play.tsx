...
// this function pulls a new flavor text to deliver to the user
  const getNewFunFact = async () => {
    ...
    .then(response => {
        ...
        setRewardText('You got a fun fact . . .' + "\n" + aFact);
    })
    .catch(error => {
        console.error(error);
    });
...
//  The cost of the multiplier and how much of an increase the multiplier gets
  ...
  return (
    <div>
      <h1 id="bigButton" onClick={buttonClick}># of clicks: {clickCounter}</h1>
      <h2 id="titleCount">Click Count: {clickCounter}</h2>
      <p id="rewardText">{rewardText}</p>
      <div>

...
function buttonClick() {
    ...
    clickCounter += aClick;
    console.log("aclick = " + clickCounter);

    // changes the tab banner and the click counter on the webpage to reflect 
    //    the amount of clicks that have occurred
    let bButton = document.getElementById("bigButton");
    if(bButton){
        bButton.innerHTML = "# of clicks: " + clickCounter;
    }

    let tCount = document.getElementById("titleCount");
    if(tCount){
        tCount.innerHTML = "Click Count: " + clickCounter;
    }
